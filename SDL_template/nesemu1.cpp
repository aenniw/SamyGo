...
namespace IO {
    ...
    void Init() {
        ...
        s = SDL_SetVideoMode(256, 240, 32, SDL_HWSURFACE + SDL_DOUBLEBUF);
                    ...
                    for (int p0 = 0; p0 < 512; ++p0)
                        for (int p1 = 0; p1 < 64; ++p1) {
                            ...
                            for (int p = 0; p < 12; ++p) // 12 samples of NTSC signal constitute a color.
                            {
                                ...
                                int r = (p + o * 4) % 12, pixel =
                                        r < 8 - u * 2 ? p0 : p1; // Use pixel=p0 to disable artifacts.
                            ...
                            if (u == 2)
                                palette[o][p1][p0] += 0x10000 * clamp(255 * gammafix(
                                        y / 1980.f + i * 0.947f / 9e6f + q * 0.624f / 9e6f));
                            if (u == 1)
                                palette[o][p1][p0] += 0x00100 * clamp(255 * gammafix(
                                        y / 1980.f + i * -0.275f / 9e6f + q * -0.636f / 9e6f));
                            if (u == 0)
                                palette[o][p1][p0] += 0x00001 * clamp(255 * gammafix(
                                        y / 1980.f + i * -1.109f / 9e6f + q * 1.709f / 9e6f));
                        }
    ...
    u8 Access(unsigned addr, u8 value, bool write) {
        ...
        if (write && addr >= 0x8000 && mappernum == 1) // e.g. Rockman 2, Simon's Quest
        {
            ...
            if (++counter == 5) {
                ...
                configure:
                cache = counter = 0;
                static const u8 sel[4][4] = {{0, 0, 0, 0},
                                             {1, 1, 1, 1},
                                             {0, 1, 0, 1},
                                             {0, 0, 1, 1}};
...
namespace PPU /* Picture Processing Unit */
{
    ...
    u8 &mmap(int i) {
        ...
        if (i < 0x2000)
            return GamePak::Vbanks[(i / GamePak::VROM_Granularity) % GamePak::VROM_Pages]
    ...
    u8 Access(u16 index, u8 v, bool write) {
        ...
        switch (index) // Which port from $200x?
        {
            ...
            case 4:
                if (write) OAM[reg.OAMaddr++] = v;        // Write or read the OAM (sprites).
            ...
            switch (scanline += 1) {
                ...
                case 241: // Begin of vertical blanking
                    ...
                    if (fp) {
                        ...
                        if (ctrlmask & 0x80) {
                            IO::joy_next[0] = fgetc(fp);
                            if (feof(fp)) IO::joy_next[0] = 0;
                        }
                        if (ctrlmask & 0x40) {
                            IO::joy_next[1] = fgetc(fp);
                            if (feof(fp)) IO::joy_next[1] = 0;
                        }
                    }
                    // Set vblank flag
                    VBlankState = 2;
...
namespace APU /* Audio Processing Unit */
{
    ...
    struct channel {
        ...
        union // Per-channel register file
        {
            ...
            RegBit<15, 1, u32> SweepEnable;   // 4003, 4007, 400B, 400F, 4010:
    ...
    void Write(u8 index, u8 value) {
        ...
        switch (index < 0x10 ? index % 4 : index) {
            case 0:
                if (ch.reg.LinearCounterDisable) ch.linear_counter = value & 0x7F;
                ch.reg.reg0 = value;
                break;
            case 1:
                ch.reg.reg1 = value;
    ...
    void tick() // Invoked at CPU's rate.
    {
        ...
        if ((hz240counter.lo += 2) >= 14915) {
            ...
            for (unsigned c = 0; c < 4; ++c) {
                ...
                if (HalfTick && c < 2 && count(ch.sweep_delay, ch.reg.SweepRate)) if (wl >= 8 && ch.reg.SweepEnable &&
                                                                                      ch.reg.SweepShift) {
                ...
                if (FullTick && c != 2 && count(ch.env_delay, ch.reg.EnvDecayRate)) if (ch.envelope > 0 ||
                                                                                        ch.reg.EnvDecayLoopEnable)
...
#undef s
        ...
        static FILE *fp = popen("resample -to mr1789800 r48000 | aplay -fdat 2>/dev/null", "w");
        fputc(sample, fp);
        fputc(sample / 256, fp);
    }
}
...
#define o(n) c(n)c(n+2)c(n+4)c(n+6)
        static void(*const i[0x108])() =
                {
                        o(00) o(0
                                        8) o(10) o(18) o(20) o(28) o(30) o(38)
