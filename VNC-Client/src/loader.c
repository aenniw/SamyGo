...
static int (*_Game_Main_)(const char *, const char *);
...
void injector(const char *path) {
    ...
    debug("- obtain game_main function address");
    *(void **) (&_Game_Main_) = dlsym(handle, "_Game_Main_");
    if (_Game_Main_ == NULL) {
        dlclose(handle);
        return;
    }
    debug("- game_main function retrieved");
    dlclose(handle);
