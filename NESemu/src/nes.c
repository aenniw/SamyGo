...
void nsf_load_bank(unsigned frame, unsigned bank) {
    ...
    if (bank == 0) {
        ...
        if (top > base) memcpy(fr, nes.rom.prg + base, top - base);
            ...
            printf("Bogus bankswitch? frame %i, bank %i, base=%x  top=%x  size=%x\n",
                   frame, bank, base, top, top - base);
    }
}
                    ...
                    if ((!in_vblank(&nes)) && (nes.ppu.control2 & 0x08)) {
                        ...
                        if (nes.ppu.v >= 0x2000) {
                            if (nes.ppu.v < 0x3F00) {
                                ...
                                Value &= 63;
                                if (trace_ppu_writes)
                                    printf("%sPalette write of %02X to %04X (mirrored to %04X)\n",
...
#endif
{
    switch (Addr & 0xE000) {
            ...
            switch ((Addr & 0x0007) | 0x2000) {
                ...
                case ppu_status: {
                    ...
                    byte tmp =
                            ...
                            ((in_vblank(&nes))
                                    << 4); /* VRAM Write flag - this is a guess. ccovel's cmcwavy demo uses this. */
                ...
                case spr_data:
                    ...
                    return nes.ppu.spriteram[nes.ppu.sprite_address] &
                           (((nes.ppu.sprite_address & 3) == 2) ? 0xE3 : 0xFF);
        ...
        case 0xC000:
        case 0xE000:
            return mapper->mapper_read(Addr);
    }
