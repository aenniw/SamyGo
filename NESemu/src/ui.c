...
void render_restorestate(struct inputctx *input) {
    ...
    if (probe_file(state_filename(&nes.rom, 1))) {
        ...
        int hover = mouseover(input, grow_rect(1.4 * ceilf(button_height - offset),
                                               drawimage(photo, px + roundf(offset), py - roundf(offset), left,
                                                         bottom)));
...
void browser_rescan(void) {
    ...
    while ((dent = readdir(dir))) {
        ...
        if (exists && S_ISREG(st.st_mode) &&
            ...
            browser_num_ents++;
        } else if (exists && S_ISDIR(st.st_mode) && (browser_num_dirs < max_dirs)) {   /* Add directory to browser */
            snprintf(filename, sizeof(filename), "%s%s%s/",
                     browser_cwd, dirsep(browser_cwd), dent->d_name);
...
void run_game_browser(struct inputctx *input) {
    ...
    if (max_y < viewport_height) {
        ...
        game_scroll = run_scrollbar(input, &file_scroller, x_base - 24,
...
void run_input_menu(struct inputctx *input) {
    ...
    for (int device = 0; device < (1 + numsticks); device++) {
        ...
        for (int joypad = 0; joypad < 4; joypad++) {
            ...
            if (mouseover(input, (SDL_Rect) {x, y + 2, 43, 47}) && (input->released & SDL_BUTTON(1)))
                next_mapping = joypad;
        }
