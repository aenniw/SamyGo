...
int DAsm(char *S, byte *A, unsigned long PC) {
    ...
    switch (AD[OP + 1]) {
        ...
        case Rl:
            J = *B++;
            PC += 2 + ((J < 0x80) ? J : (J - 256));
            sprintf(S, "%s $%08lX", MN[AD[OP]], PC);
            break;

        case Im:
            sprintf(S, "%s #$%02X", MN[AD[OP]], *B++);
            break;
        case Zp:
            sprintf(S, "%s $%02X", MN[AD[OP]], *B++);
            break;
        case Zx:
            sprintf(S, "%s $%02X,x", MN[AD[OP]], *B++);
            break;
        case Zy:
            sprintf(S, "%s $%02X,y", MN[AD[OP]], *B++);
            break;
        case Ix:
            sprintf(S, "%s ($%02X,x)", MN[AD[OP]], *B++);
            break;
        ...
        case Ab:
            sprintf(S, "%s $%04X", MN[AD[OP]], B[1] * 256 + B[0]);
            B += 2;
            break;
        case Ax:
            sprintf(S, "%s $%04X,x", MN[AD[OP]], B[1] * 256 + B[0]);
            B += 2;
            break;
        case Ay:
            sprintf(S, "%s $%04X,y", MN[AD[OP]], B[1] * 256 + B[0]);
            B += 2;
            break;
        case In:
            sprintf(S, "%s ($%04X)", MN[AD[OP]], B[1] * 256 + B[0]);
