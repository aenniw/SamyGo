...
void ntsc2x_emitter(unsigned line, byte *colors, byte *emphasis) {
    ...
    for (int i = 0; i < 640; i++) {
        ...
        dbl[i * 2 + 0] = x;
        dbl[i * 2 + 1] = ((x >> 1) & 0x7F7F7F7F) + ((y >> 1) & 0x7F7F7F7F) +
                         ((((x & 0x01010101) + (y & 0x01010101)) >> 1) & 0x7F7F7F7F);
    }
                ...
                for (int x = 0; x < 22; x++) {
                    for (int decimation_phase = 0; decimation_phase < 2; decimation_phase++) {
                        for (int component_index = 0; component_index < 4; component_index++) {
                            ...
                            for (int alignment = 0; alignment < 3; alignment++)
                                rgb_output[emphasis][alignment][color][decimation_phase][x][component_index] = llround(
                                        tmp / 3.0);
                        }
                    }
                }
